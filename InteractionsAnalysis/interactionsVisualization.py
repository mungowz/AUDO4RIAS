...
def retriveInteractions(scores, protein, residues):
    close_contacts = []
    hydrogen_bonds = []
    for res in residues:
    ...
    return close_contacts, hydrogen_bonds



def plotInteraction(protein, scores, contact_states):
    residues = [key for key in scores[protein].keys()]
    close_contacts, hydrogen_bonds = retriveInteractions(scores, protein, residues)
...
def buildHeatmap(title, contact_states, ligands, residues):
    bond = []
