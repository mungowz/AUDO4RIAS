...
class Ligands(CTkFrame):
    ...
    def __init__(self, parent, controller):
        ...
        label_menu = CTkLabel(master=frame_info, text=Ligands.MENU, height=2, corner_radius=1, fg_color=("white", "gray38"), justify=LEFT)
        label_menu.grid(column=0, row=0, sticky="nwe", padx=15, pady=15)

        label_input_file = CTkLabel(master=frame_right, height=1, text="Specify the path of the input file or leave it blank to use the default path")
        ...
        check_box.grid(row=15, column=0, pady=10, padx=20, sticky="w")
        ...
        button_execute = CTkButton(master=frame_left, text="Execute", command=lambda: controller.execute_ligands(
                entry_input_file.get(),
                entry_excel_folder.get(),
                entry_sdf_folder.get(),
        ...
        button_help = CTkButton(master=frame_left, text="Help", command=lambda: controller.help(Ligands))
